...
from config import (LIB_FILETYPE, LIB_PATH, REC_FILETYPE, REC_FOLDER_RAW,
                    REC_FOLDER_TRIM, REC_OVERLAP, SAMPLE_RATE, CHANNELS, REC_NICE, REC_BUFFER, REC_FORMAT)
...
class RecordingProcess:
    def __init__(self, library_path=LIB_PATH, filetype=LIB_FILETYPE, stream_type='Realtime', buffer_len=REC_OVERLAP, led: StatusLed = None, db: MusicDatabase = None):
        self.led = led if led is not None else StatusLed()
    ...
    def start_recording(self, use_alt=False):
        ...
        self.led.set_rec_status(self.device, 'Start')
    ...
    def parse_rtp(self, rtp, ms):
        ...
        if self.song_data.length.value and left and left > 0:
            ...
            self.led.set_progress(progress)
        if not is_good:
        ...
        if left < 0 and self.ms_since_start() > 10_000 and self.is_recording:
            self.led.set_rec_status(self.device, 'Done')
    ...
    def parse_progress(self, start, end):
        if (self.rtp_type is None or self.rtp_type == 'firstframe'):
            ...
            if self.is_recording:
                self.led.set_rec_status(self.device, 'Good')
    ...
    def trim_wav(self):
        ...
        try:
            ...
            self.led.set_rec_status(self.device, 'Error')
            return None, 0
    ...
    def save_song(self):
        self.led.set_rec_status(self.device, 'Save')
        self.led.update()
        if self.has_saved:
            print('already saved')
            return
        ...
        if trimmed_path is None:
            ...
            self.delete_temp_files()
            return
        if self.song_data.length.value > 0:
            if duration < self.song_data.length.value * 0.9:
                ...
                self.led.set_rec_status(self.device, 'Off')
                self.led.set_db_status(self.device, "SAVED")
                self.led.update()
                time.sleep(0.2)
